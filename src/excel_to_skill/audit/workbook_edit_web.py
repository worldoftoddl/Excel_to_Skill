...
class AuditWorkbookEditHttpAdapter:
    ...
    def __init__(self, service: WorkbookEditService) -> None:
    ...
    *,
    ...
    async def get_endpoint(workflow_id, request):
        ...
        response = await offload(
            request,
            lambda: adapter.get_workflow(principal=principal, workflow_id=workflow_id),
        )
        return response_value(response)
    ...
    error_responses = {
        ...
        for status in (400, 403, 404, 409, 413, 422, 500, 503)
    }
