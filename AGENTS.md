...
turn-scoped supplemental responses without checkpoint or prepared-artifact authority. The current
complete suite is `514 passed, 1 skipped`; compileall, lock consistency, diff/credential-pattern
...
Aggregate-bound routing normally uses no child LLM, but the same isolated child selector runs when
opt-in research is selected for an exact exposed source scope. Use synthetic data for live tests and
