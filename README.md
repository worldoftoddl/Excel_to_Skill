...
processing pipeline이 게시한 bundle은 서버가 workbook/sheet/aggregate 대화 root를 함께
고정한다. 클라이언트가 scope를 생략하면 이 binding을 자동 주입하고, 다른 sheet나 aggregate로
바꾸어 요청하면 모델 호출 전에 `BUNDLE_SCOPE_CONFLICT`로 거부한다. Immutable package의 read
lock과 conversation commit lock은 package object가 아니라 private runtime에 기록하므로 대화가
게시 snapshot을 변경하지 않는다.
...
이 API 자체는 raw source 보관 경계일 뿐 convert, prepare, aggregate 또는 LLM을 실행하지 않는다.

### Processing pipeline API

`audit.processing`은 위 immutable raw snapshot을 기존 convert/verify, scope planner,
prepare/aggregate, conversation service에 연결한다. `audit.processing_sqlite`는 job·idempotency·
lease/fence·published bundle catalog를 보존하고, `audit.processing_store`는 완전히 검증된 package를
content manifest와 함께 private immutable snapshot으로 원자 게시한다. strict HTTP 경계는
`audit.processing_web.create_processing_fastapi_app(...)`이다.

```text
POST /v1/audit/workbooks/{workbook_id}/raw-snapshots/{raw_snapshot_id}/processing-jobs
POST /v1/audit/processing-jobs/{job_id}/scope-selection
GET  /v1/audit/processing-jobs/{job_id}
```

첫 POST는 외부 모델이나 MCP를 만들지 않고 raw digest 확인, deterministic convert, source를 붙인
full verify와 `audit_scope_plan`까지만 실행한 뒤 `awaiting_scope`를 반환한다. scope 선택 body는
다음 세 mode 중 하나이며 반드시 응답의 정확한 `scope_plan_sha256`에 묶인다.

- `workbook`: 전체 workbook을 한 bundle로 prepare하며 `scope_ids`는 비운다.
- `all_sheets`: analyzable sheet를 모두 독립 prepare한 뒤 2개 이상이면 exact-sheet aggregate를
  만든다. 현재 한 job의 시트 상한은 64개이며 plan의 `all_sheets.selectable`과
  `selection_limit`으로 미리 확인한다.
- `selected_sheets`: plan에서 관찰한 analyzable scope ID를 1~64개 선택한다. 한 시트면 sheet
  binding, 여러 시트면 선택 집합만 포함하는 aggregate binding을 게시한다.

Job 상태는 `planning → awaiting_scope → preparing → aggregating → published/failed`다. 각 POST는
정확히 하나의 `Idempotency-Key`를 요구한다. 긴 model/MCP 호출 중에도 SQLite lease를 heartbeat로
갱신하고, 동일 job staging은 process 간 private file lock으로 직렬화한다. Worker가 죽으면 만료된
lease를 더 높은 monotonic fence로 재확보해 deterministic/cache 단계부터 재검증한다. 명시적
terminal failure retry API는 아직 없으므로 `failure.retryable`은 현재 항상 false다.

선택 scope 중 하나라도 실패하거나 final verify/aggregate commit gate가 실패하면 bundle row를
게시하지 않는다. 성공 시 package readback, immutable store publication, principal-scoped bundle row와
job `published` 전이를 거친다. 대화 resolver는 매번 package manifest와 sheet/aggregate commit을
다시 검증한다. Raw bytes가 별도 보존정책으로 만료돼도 committed bundle의 일반 대화는 가능하며,
그 경우 optional raw-range inspection provider만 비활성화된다. 현재 FastAPI adapter는 bounded
executor에서 동기 실행한다. 비동기 queue, distributed workspace, orphan snapshot GC는 운영
배포에서 추가할 host 책임이다.
