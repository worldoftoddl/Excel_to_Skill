...
citation만 사용하고 MCP를 다시 호출하지 않는다. aggregate main-agent도 별도 child LLM을
호출하지 않는다. 한 대화 모델이 opaque ref와 도구 action만 선택하고, aggregate에서 exact
source sheet로 내려가는 검색·조회·trace는 애플리케이션의 결정론 reader가 실행한다. 다음
orchestration 단계는 committed context 밖의 질문에만 명시적으로 여는 dynamic standards
research subgraph다. 이 worker가 도입되더라도 CID 선택만 맡기고, 애플리케이션이 원문을 다시
직조회·검증하며 결과는 prepared bundle 밖의 turn-scoped `ephemeral` 근거로 유지해야 한다.
