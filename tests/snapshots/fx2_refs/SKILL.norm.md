---
name: fx2-refs-e85b28393b5c
description: "스프레드시트 2매 — (빈 시트) / (빈 시트). (의미 주석 미승인)"
---
...
- `S1` (`layout/S1.html`, used range `A1:A3`) — (머리 텍스트 없음)
- `S2` (`layout/S2.html`, used range `A1:B3`) — (머리 텍스트 없음)
...
- 대표:
  - `S2!B1` → `S1!A1` (cell)
  - `S2!B2` → `S1!A1:A3` (range)
- observability: 통합문서 전체 관찰됨
- 미해결(INDIRECT/OFFSET 등): 1건
