...
1. **§6 승계(오너 확정: `version_changed`만)** — `converter_version`만 올라 재변환할 때만 승계한다. `force`·`sha_mismatch`는 제외. **주의: probe는 version을 params보다 먼저 보므로, 버전+옵션(`max_rows`·`full_names`)이 동시에 바뀌어도 reason이 `version_changed`로 나온다** → `_should_inherit`가 `entry.conversion_params == conv_params`를 명시 재확인해 옵션이 다르면 승계하지 않는다(검수 보정). 완료 조건: 기존 패키지 실재 + `data/semantics.json` + sha 일치 + **패키지 내부 `meta.annotation.annotation_key`가 non-null이고 `_index.annotation_key`와 일치**(완료 marker 권위는 4a대로 패키지 내부 — `_index`만으로 판정하지 않는다).
