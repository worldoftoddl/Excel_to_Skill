...
1. **§6 승계(오너 확정: `version_changed`만)** — `converter_version`만 올라 재변환할 때(`probe.reason=="version_changed"`)만 승계한다. `force`·`params_changed`·`sha_mismatch`는 제외. 조건: 기존 패키지 실재 + `data/semantics.json` + 기존 `_index.annotation_key` non-null(완료 주석) + sha 일치.
