...
1. **§7 `review` 서브커맨드** — `review <패키지> (--approve | --reject --note "사유")`. `--approve`는 **승인 전 `verify` 전체 통과가 전제**(오너 확정 — V2만이 아니라 V1·필수파일·full_names 일관성까지; 깨진 패키지는 승인 불가) + **완료된 주석만 승인**(v1.8 — semantics.generator+meta로 계산한 4성분 키가 **`meta.annotation.annotation_key`(패키지 내부 완료 marker)**와 일치해야; partial annotate는 키를 안 남기므로 verify가 통과해도 승인 거부. `_index`가 아니라 패키지 파일만 보므로 **폴더를 옮겨도 승인 동작**한다 — `annotation_key`는 meta에 두고 `_index.annotation_key`는 캐시/승계용 미러), 통과 시 `review={status:approved, reviewed_at, note:null}`로 갱신하고 승인판 SKILL.md 재생성. `--reject`는 `--note` 필수, `{rejected, reviewed_at, note}`로 갱신하고 SKILL.md를 **미승인 형태로 재생성**(approve→reject 시에도 ⑥가 승인 의미를 노출하지 않음 — 상태와 SKILL 항상 일치).
...
1. **§6 주석 캐시** — `annotation_key = sha256(file_sha + annotator_version + model + prompt_sha)`(4성분 해시, `cache.annotation_key`). `annotate <패키지>`가 `meta.source.sha256`으로 키를 만들고, `_index.json` 항목의 `annotation_key`가 같고 `semantics.json`이 있으면 **재주석 생략(LLM·클라이언트 미생성)** — `--force`로 무시. 재주석하면 `cache.update_annotation`으로 항목의 `annotation_key`+`review_status="draft"` 기록. review는 `review_status`(approved/rejected)를 `_index.json`·`meta.annotation`·`semantics.review` 세 곳에 함께 반영(단일 진실 유지). 모델·프롬프트·어노테이터·파일이 바뀌면 키가 달라져 miss→재주석. **캐시 hit는 완료된 주석에만 허용** — 단위 실패(`excluded` 있음)면 `annotation_key`를 기록하지 않고 기존 키도 **clear(None)**해, 실패/부분 결과가 캐시를 오염시켜 재시도를 막는 일이 없게 한다(`--force` 재주석이 부분 실패해도 stale hit 방지).
2. **meta↔semantics 일관성 검사(`annotation`)** — verify에 검사 추가(원본 불요·항상 수행): `semantics.json`이 있으면 `meta.annotation.present==true` + `review_status==semantics.review.status` + `annotator_version==semantics.generator.annotator_version`, 없으면 `present==false`·`annotation_key==null`. 어긋나면 실패. **완료 marker 검증**: `annotation_key`가 non-null이면 `semantics.generator+meta.source.sha256`으로 재계산한 4성분 키와 일치해야(훼손·가짜 완료 키 주입 검출); `null`이면 partial(미완료) draft로 **허용**하되 **`review_status=="approved"`인데 `annotation_key==null`이면 실패**(approved는 완료 상태이므로 marker가 반드시 있어야; v1.9 보정). `meta.annotation`을 운영 필드로 쓰기로 한 이상, 캐시/승계가 읽기 전에 출처 간 일치를 verify가 강제한다. 완료 marker는 **패키지 내부(`meta.annotation.annotation_key`)** 에 두고 `_index.annotation_key`는 캐시/승계용 미러로 둔다 → approve·verify가 `_index` 없이 패키지 파일만으로 동작(폴더 이동에도 견고).

---

## 개정 이력 (v1.8 → v1.9, M3 4b단계 — 승계 규칙)

M3 4단계 두 번째 하위 단계로, §6 승계 규칙을 `convert` 원자적 교체에 배선한다. 32종 시드(4c)는 후속.

1. **§6 승계(오너 확정: `version_changed`만)** — `converter_version`만 올라 재변환할 때만 승계한다. `force`·`sha_mismatch`는 제외. **주의: probe는 version을 params보다 먼저 보므로, 버전+옵션(`max_rows`·`full_names`)이 동시에 바뀌어도 reason이 `version_changed`로 나온다** → `_should_inherit`가 `entry.conversion_params == conv_params`를 명시 재확인해 옵션이 다르면 승계하지 않는다(검수 보정). 완료 조건: 기존 패키지 실재 + `data/semantics.json` + sha 일치 + **`meta.annotation.annotation_key` == `_index.annotation_key` == 재계산 키**(semantics.generator+meta.source.sha256으로 재계산 — approve·verify와 **동일 완료 기준**. generator/키 조합이 훼손돼 재계산과 안 맞으면 승계 거부해, verify가 이미 실패시키는 semantics가 새 패키지로 번지지 않게 한다; v1.9 보정).
2. **이월 배선(`_convert_one`)** — 구 패키지 삭제 전에 구 `semantics.json`을 읽어 staging(새 결정론 계층)으로 이월 → **V2 재검증**(이월 evidence를 새 `meta.sheets[].dimensions`로 `collect_evidence_problems`). 통과면 원 상태 유지(approved면 approved), **실패면 오너 확정대로 `review.status="draft"` 강등 + `review.note`에 사유**(`annotation_key`는 완료 marker라 유지 — 재승인은 approve의 verify 게이트가 V2로 다시 막음). 이월 상태에 맞춰 `meta.annotation`(present·review_status·annotator_version·annotation_key)과 SKILL.md(`build_skill_md_from_package` — approved면 승인판)를 재생성하고, 원자적 교체 후 `cache.record`에 이월된 `annotation_key`·`review_status`를 기록. 주석 없는 패키지·`--force`는 종전대로 새 결정론 패키지만 생성(승계 없음).
...
## 개정 이력 (v1.10 → v1.11, M3 4c — 배치 명령·입력 예산 발췌)

M3 4c 운영 준비. `annotate --all` 배치와, 실 코퍼스 검증에서 드러난 컨텍스트 초과를 어노테이터 입력 예산으로 해결한다.

1. **`annotate --all <converted_root>`** — §3 참조. `_index.json` 등록 항목 기준 전 패키지 일괄 주석(캐시 hit 생략·파일별 실패 격리·폴더/meta 누락도 failed·4범주 집계). stdout=요약 1줄(패키지명 미포함), exit=실패>0∨제외>0이면 1.
2. **캐시 hit 본문 계약 확인** — 재주석 생략은 키 3자 일치(`_index`==`meta.annotation`==`semantics.generator` 재계산==현재 key)에 더해 **저장된 semantics가 지금도 스키마 V1 + V2를 통과**할 때만. 훼손·부분·본문 위반은 miss로 재주석("annotate 산출물은 항상 verify 통과"를 캐시 경로에서도 보장).
3. **시트 layout 입력 예산 발췌(§7)** — 시트 단위 프롬프트의 layout HTML을 보수적 char 예산(`_LAYOUT_BUDGET`)으로 **행 경계 발췌**(앞+뒤 행 보존, 가운데 `(…행 생략)` 마커, 모델에 발췌 고지 → 미관찰 영역 주장 금지). 그래도 컨텍스트가 초과되면 더 작은 예산(`_LAYOUT_BUDGET_MIN`)으로 **1회 축소 재시도**, 최종 초과면 **그 시트만 제외**(나머지 시트·워크북은 계속 — 부분 결과는 완료 키 없음·승인 불가·exit 1). **컨텍스트 초과만 구분**하고 그 외 오류는 그대로 전파. 입력 정책 변경이므로 **`ANNOTATOR_VERSION` 0.1.0→0.2.0**(기존 주석 캐시 무효화). 실 코퍼스 최대 입력 2건(각 ~13MB·시트 5개)이 발췌 후 excluded=0·V2 PASS 확인.

---

## 개정 이력 (v1.11 → v1.12, M3 종료 검수 — Agent 소비 인터페이스)

M3 종료 기준을 "semantics가 정연한가"가 아니라 **"Agent가 조서를 정확·신속하게 이해하는가"**로 재정의(오너). 그 전제로, JSON을 통째로 컨텍스트에 붓지 않고 단계 조회하는 소비 계층을 먼저 고정한다.

1. **소비 인터페이스 `consume.py` + CLI(§3·§10)** — overview/inspect/search/refs 4종. 결정론(anthropic 무관), 패키지 파일만 읽음.
2. **승인 전 은닉 준수** — overview는 `review.status=='approved'`일 때만 해석 내용을 노출(draft=상태·건수, rejected=상태만). 평가용 승인판은 임시 복사본에서만 만든다(원본 draft·meta·index 불변).
3. **출력 예산 강제** — 모든 `--limit`은 `1..HARD_LIMIT`로 클램프(음수·초대형 우회 차단), overview 워크북 개요는 구간 상세를 담지 않고 `overview --sheet S`로 2단계 분리, 전 결과에 returned·total·truncated.
4. **손상·잘못된 입력 = exit 1** — 파일 누락·JSON 손상·손상 JSONL 행·주소 문법(A:B 등)·시트 부재를 ConsumeError로 통일(조용한 빈 결과·크래시 금지).
5. **승인판 SKILL ⑥ 요약화** — 구간·근거·필드 상세를 SKILL에 전부 싣지 않고(통독 시 단계 조회 무력화) 워크북 주장·시트 purpose·구간 수 요약만, 상세는 소비 명령으로 안내.
6. **SKILL ⑤ 근거 문구 완화** — "모든 문장에 셀 주소"를 "셀 내용·문서 의미 주장엔 시트!셀 근거, 구조 정보는 overview 필드 근거"로 좁힘.
7. **셀 뷰 필드** — 최소 sheet·cell·value·formula + data_type·number_format·bold·border·fill·cached_value·merged_range(있을 때만; null·false 생략). refs.reason 유지. 빈 입력란·날짜·백분율·금액 구분 지원.

다음: 대표 6종 자동 선정(오버사이즈 2·xls 1·멀티시트·참조집중·입력란집중) → A/B 이해도 파일럿(조건명 은닉·교차 배치·새 컨텍스트, 질문·정답은 구조 데이터+오너 검토로 독립 작성, 조회 횟수·바이트·시간 기록) → 하네스 보정 → 32종 공통 평가 → M3 종료 판단. LangSmith 오프 유지.
...
excel-to-skill overview <패키지경로> [--sheet S] [--limit N]
excel-to-skill inspect  <패키지경로> --sheet S [--range A1:B10 | --cell A1] [--limit N]
excel-to-skill search   <패키지경로> --query T [--sheet S] [--limit N]
excel-to-skill refs     <패키지경로> --cell S!A1 [--limit N]
```

- **소비 인터페이스(§10, overview/inspect/search/refs)**: Agent가 원본 JSON을 통째로 읽지 않고 **개요 → 시트 → 셀**로 단계 조회하는 읽기 계층(결정론). `overview`는 셀 원문 없이 구조·상태·카운트 + (semantics가 **approved**일 때만) 워크북 주장·시트 purpose·구간 수·의미유형 집계를 반환하고, 구간 상세는 `overview --sheet S`로 분리한다(draft는 상태·건수만, rejected는 상태만 — 승인 전 은닉 계약). `inspect`는 지정 범위 셀, `search`는 값·수식 부분일치, `refs`는 지정 셀의 출입 엣지만 반환한다. 모든 결과는 **하드 상한(`_clamp_limit`, 최대 `HARD_LIMIT`)** + `returned`·`total`·`truncated`를 적용하고, 셀 레코드는 최소 `sheet`·`cell`·`value`·`formula`(+ 있을 때만 data_type·number_format·bold·border·fill·cached_value·merged_range)를 담는다. 파일 누락·JSON 손상·주소 문법 오류·시트 부재는 조용한 빈 결과가 아니라 exit 1(ConsumeError)다. SKILL.md ⑤가 이 조회 방법을 명시하고, 승인판 ⑥도 요약만 실어 SKILL 통독으로 단계 조회가 무력화되지 않게 한다.
